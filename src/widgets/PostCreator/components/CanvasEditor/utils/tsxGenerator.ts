...
// Generates React TSX code from the canvas elements
export const generateTsx = (elements: CanvasElement[], canvas?: Canvas) => {
...
${indent}}`;
        ...
        // Generate appropriate TSX based on element type
        let tsxContent = '';
...
${indent}</p>`;
          ...
          // For image elements with TypeScript props
          tsxContent = `
${indent}<img 
...
${indent}</div>`;
  ...
  const elementsTsx = renderElements(elements);
  
  // Component with TypeScript interfaces and type annotations
