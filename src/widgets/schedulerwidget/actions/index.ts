...
export const deleteSeries = (seriesId: number) => async (dispatch: any): Promise<ActionResponse> => {
    try {
        const response = await axios.delete(`${API_BASE_URL}/series/${seriesId}`, {
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
                'Content-Type': 'application/json',
                'Accept': 'application/json'
...
// Fetch Series Events
export const fetchSeriesEvents = (seriesId: number) => async (dispatch: any): Promise<void> => {
...
export const createEvent = (eventData: any) => async (dispatch: any): Promise<ActionResponse> => {
...
// Add Event Time
export const addEventTime = (timeData: any) => async (dispatch: any): Promise<ActionResponse> => {
    try {
        const response = await axios.post(`${API_BASE_URL}/eventTimes`, timeData, {
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
                'Content-Type': 'application/json',
                'Accept': 'application/json'
...
// Search Songs
export const searchSongs = (keyword: string) => async (dispatch: any): Promise<void> => {
...
// Get Event Songs
export const getEventSongs = (eventId: number) => async (dispatch: any): Promise<void> => {
    dispatch({ type: ActionTypes.FETCH_EVENT_SONGS_REQUEST });
    
    try {
        const response = await axios.get(`${API_BASE_URL}/eventSongs/${eventId}`, {
            headers: {
                'X-CSRF-TOKEN': getCSRFToken(),
                'Content-Type': 'application/json',
                'Accept': 'application/json'
...
// Add Song to Event
export const addEventSong = (eventId: number, songId: number, orderStatus: number) => async (dispatch: any): Promise<ActionResponse> => {
    dispatch({ type: ActionTypes.ADD_EVENT_SONG_REQUEST });
    
    try {
        const response = await axios.post(`${API_BASE_URL}/eventSongs`, {
            event_id: eventId,
            song_id: songId,
            order: orderStatus
        }, {
        ...
        if (response.data.status) {
            dispatch({
                type: ActionTypes.ADD_EVENT_SONG_SUCCESS,
                payload: response.data.result
            });
